...
from .corrupted import CorruptedSST
from .sst import SST
...
def init_dataset(corruption, _name, batch_size, num_workers, drop_last, shuffle, size='all', pin_memory=True, **kwargs):
    ds = get_dataset_by_name(_name)(**kwargs)
    t = get_output_transform(_name)

    if corruption is not None:
        ds = CorruptedSST(ds, corruption, t)
