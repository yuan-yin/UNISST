...
def preprocess(videos, target_resolution, cast=False):
...
def fvd_interface(real, fake):
  '''
  [batch_size, num_frames, height, width, depth]
  '''
  real = real.permute(0, 2, 3, 4, 1)
  fake = fake.permute(0, 2, 3, 4, 1)

  real = torch.clamp(real, min=-1, max=1)
